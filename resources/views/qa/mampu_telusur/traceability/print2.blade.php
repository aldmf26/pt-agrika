                        ...
                        <tbody>
                            ...
                            @foreach ($bk as $b)
                                @php
                                    ...
                                    $kiriman = $kirim[$b['nm_partai']] ?? [];
                                    $jmlRow = max(1, count($kiriman)); // kalau belum ada kirim, minimal 1 row
                                @endphp

                                @for ($i = 0; $i < $jmlRow; $i++)
                                    <tr>
                                        @if ($i == 0)
                                            {{-- tampilkan data utama dari $bk sekali saja, merge dengan rowspan --}}
                                            <td rowspan="{{ $jmlRow }}" class="text-center">{{ $loop->iteration }}
                                            </td>
                                            <td rowspan="{{ $jmlRow }}">{{ tanggal($b['tgl_panen']) }}</td>
                                            @php
                                                $tgl = date('Y-m-d', strtotime($b['tgl_panen'] . ' + 1 days'));
                                                $rumah_walet = DB::table('rumah_walet')
                                                    ->where('id', $b['rwb_id'])
                                                    ->first();
                                            @endphp
                                            <td rowspan="{{ $jmlRow }}">{{ tanggal($tgl) }}</td>
                                            <td rowspan="{{ $jmlRow }}">{{ $rumah_walet->no_reg }}</td>
                                            <td rowspan="{{ $jmlRow }}">{{ $rumah_walet->nama }}</td>
                                            <td rowspan="{{ $jmlRow }}">{{ $b['no_invoice'] }}</td>
                                            <td rowspan="{{ $jmlRow }}" class="text-end">
                                                {{ number_format($b['gr_kotor'], 0) }}</td>
                                            <td rowspan="{{ $jmlRow }}" class="text-end">
                                                {{ number_format($b['berat_bersih'], 0) }}</td>
                                            <td rowspan="{{ $jmlRow }}" class="text-end">
                                                {{ number_format($b['gr_kotor'] - $b['berat_bersih'], 0) }}</td>

                                            {{-- kolom lain dari proses, grading dll --}}
                                            <td rowspan="{{ $jmlRow }}" class="text-end">
                                                {{ number_format($b['gr_awal'], 0) }}</td>
                                            <td rowspan="{{ $jmlRow }}">{{ tanggal($b['tgl_terima']) }}</td>
                                            <td rowspan="{{ $jmlRow }}" class="text-center">
                                                {{ number_format($b['pcs_awal'], 0) }} /
                                                {{ number_format($b['gr_awal'], 0) }}</td>
                                            <td rowspan="{{ $jmlRow }}" class="text-center">
                                            ...
                                            <td rowspan="{{ $jmlRow }}" class="text-center">
                                                {{ tanggal($b['tgl_selesai_ctk']) }}</td>
                                            <td rowspan="{{ $jmlRow }}" class="text-center">
                                                {{ number_format($b['pcs_awal_ctk'], 0) }} /
                                                {{ number_format($b['gr_awal_ctk'], 0) }}</td>
                                            <td rowspan="{{ $jmlRow }}" class="text-center">
                                            ...
                                            <td rowspan="{{ $jmlRow }}" class="text-center">
                                                {{ tanggal($b['tgl_grading']) }}</td>
                                            <td rowspan="{{ $jmlRow }}" class="text-center">
                                                {{ number_format($b['pcs_grading'], 0) }}</td>
                                            <td rowspan="{{ $jmlRow }}" class="text-center">
                                                {{ number_format($b['gr_grading'], 0) }}</td>
                                            <td rowspan="{{ $jmlRow }}" class="text-center">
                                                {{ tanggal($b['tgl_grading']) }}</td>
                                            <td rowspan="{{ $jmlRow }}" class="text-center">
                                                {{ number_format($b['pcs_grading'], 0) }}</td>
                                            <td rowspan="{{ $jmlRow }}" class="text-center">
                                                {{ number_format($b['gr_grading'], 0) }}</td>
                                            <td rowspan="{{ $jmlRow }}" class="text-center">
                                                {{ number_format(($b['gr_grading'] / max(1, $b['gr_awal'])) * 100, 0) }}%
                                            </td>
                                            <td rowspan="{{ $jmlRow }}" class="text-center">
                                                {{ number_format($b['pcs_grading'], 0) }}</td>
                                            <td rowspan="{{ $jmlRow }}" class="text-center">
                                            ...
                                            @endphp
                                            <td rowspan="{{ $jmlRow }}" class="text-center">{{ tanggal($tgl) }}
                                            </td>
                                        @endif

                                        {{-- bagian looping pengiriman --}}
